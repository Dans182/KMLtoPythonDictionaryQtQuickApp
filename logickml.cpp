...
void LogicKml::writeDictionaryFile(const QString &outputFileName, const QDomNodeList &placemarkers, QString &kmlFileNameWithoutExtension)
{
    ...
    if (kmlFileNameWithoutExtension == "uncover_events") {
        writeUncoverEvents(placemarkers, txtStream, count);
    }
    else if (kmlFileNameWithoutExtension == "roadType") {
        writeRoadType(placemarkers, txtStream, count);
    }
    else if (kmlFileNameWithoutExtension == "recom_speeds") {
        writeRecomSpeeds(placemarkers, txtStream, count);
    }
    else if (kmlFileNameWithoutExtension == "next_speed_limits") {
        writeNextSpeedLimits(placemarkers, txtStream, count);
    }
    else if (kmlFileNameWithoutExtension == "3d_groups") {
        write3DGroups(placemarkers, txtStream, count);
    }
    else if (kmlFileNameWithoutExtension == "filter_events") {
        writeFilterEvents(placemarkers, txtStream, count);
    }
    else if (kmlFileNameWithoutExtension == "id_crosswalk") {
        writeIDCrosswalk(placemarkers, txtStream, count);
    }
...
void LogicKml::writeUncoverEvents(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
    //uncover_events.kml
...
void LogicKml::writeRoadType(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
    //roadType.kml
...
void LogicKml::writeRecomSpeeds(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
    //recom_speeds.kml
    //# 1: {'lat': 0.058750776, 'lon': 0.008640828, 'crs': 20, 'type': "incio_circuito_1",'dist_to_next': 0, 'current_dist': 0},
    for (int i = 0; i < placemarkers.count(); ++i) {
...
void LogicKml::writeNextSpeedLimits(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
...
void LogicKml::write3DGroups(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
    //3d_groups.kml
    //# 1: {'lat': 0.058750776, 'lon': 0.008640828, 'crs': 20, 'type': "incio_circuito_1",'dist_to_next': 0, 'current_dist': 0},
...
void LogicKml::writeFilterEvents(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
    //filter_events.kml
    //#{ 1: { 'lat': 0.0172808243863708, 'lon': 0.0677908626541539, 'event': "recto", 'value_can': 1, 'type': "A", 'dist_to_next': 11000, 'current_dist': 0},
    for (int i = 0; i < placemarkers.length(); ++i) {
...
void LogicKml::writeIDCrosswalk(const QDomNodeList &placemarkers, QTextStream &txtStream, int &count)
{
    //id_crosswalk.kml
    //# { 1: { 'lat': 0.0172808243863708, 'lon': 0.0677908626541539, 'event': "recto", 'value_can': 1, 'type': "A", 'dist_to_next': 11000, 'current_dist': 0},
    for (int i = 0; i < placemarkers.length(); ++i) {
